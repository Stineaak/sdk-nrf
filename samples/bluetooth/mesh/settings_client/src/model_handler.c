...
int bt_mesh_TEST_cli_set(struct bt_mesh_onoff_cli *cli,
			  ...
			  struct bt_mesh_onoff_status *rsp)
{
	...
	net_buf_simple_add_u8(&msg, set->val);
...
int bt_mesh_TEST_cli_set_unack(struct bt_mesh_onoff_cli *cli,
				struct bt_mesh_msg_ctx *ctx,
				const struct bt_mesh_TEST_set *set)
{
...
static void button_handler_cb(uint32_t pressed, uint32_t changed)
{
	...
	for (int i = 0; i < 4; ++i) { 
		...
		if (bt_mesh_model_pub_is_unicast(buttons[i].client.model)) {
			printk("Model publication is Unicast");
			err = bt_mesh_TEST_cli_set(&buttons[i].client, NULL, 
						    &set, NULL); 
		} else {
			err = bt_mesh_TEST_cli_set_unack(&buttons[i].client,
							  NULL, &set);
			if (!err) {
